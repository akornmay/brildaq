        ...
        if (pollItems[0].revents & ZMQ_POLLIN) {
            ...
            if (m_ls > old_ls or m_run > old_run) {
                ...
                for (unsigned i = 0; i < nChannels; ++i) {
                    ...
                    avgRateRaw += rate/nChannels;
                    avgRateNum += channelEff < 1. ? 1./(channelEff*(1 - channelEff)) : 1.;
                    avgRateDen += rate > 0 ? 1./(rate*(1 - channelEff)) : 1.;
                ...
                streamer.insert_field(payload->payloadanchor, "avgraw", &pZeroRaw);
